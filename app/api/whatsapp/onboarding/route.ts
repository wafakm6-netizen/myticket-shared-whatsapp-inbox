...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // This route now handles the direct Business Login redirect flow, so the
    // exchange must use the exact same redirect URI that created the code.
    ...
    if (!tokenResponse.ok || !tokenPayload?.access_token) {
      console.error('[whatsapp-onboarding] token exchange failed', { httpStatus: tokenResponse.status, code: tokenPayload?.error?.code, subcode: tokenPayload?.error?.error_subcode, type: tokenPayload?.error?.type, message: tokenPayload?.error?.message })
      return NextResponse.json({ error: tokenPayload?.error?.message || 'Meta token exchange failed.', metaCode: tokenPayload?.error?.code || null, metaSubcode: tokenPayload?.error?.error_subcode || null }, { status: 502 })
    }
    ...
    if (wabaId) {
      ...
      if (!subscribeResponse.ok || subscribePayload?.success === false) {
        console.error('[whatsapp-onboarding] WABA app subscription failed', { httpStatus: subscribeResponse.status, code: subscribePayload?.error?.code, message: subscribePayload?.error?.message, wabaId })
    ...
    console.info('[whatsapp-onboarding] completed', { wabaId: wabaId || null, phoneNumberId: phoneNumberId || null, subscribed: Boolean(wabaId) })
    return NextResponse.json({ success: true, wabaId: wabaId || null, phoneNumberId: phoneNumberId || null, subscribed: Boolean(wabaId) })
  } catch (error) {
